...
function convertToKebabCase(obj) {
  ...
  for (const key in obj) {
      if (obj.hasOwnProperty(key)) {
          const newKey = key
              .replace(/([a-z])([A-Z])/g, '$1-$2') // 将驼峰命名法转换为中划线
              .replace(/_/g, '-')                 // 将下划线转换为中划线
              .toLowerCase();                     // 转换为小写
...
export function pureParams(params:any,deleteZero:boolean = false,method:string='GET'){
  let realParams:any = {}
  Object.keys(params).forEach(key=>{
    ...
    if(method==='GET'){
      realParams=convertToKebabCase(realParams)
    }
  })
  return realParams
}